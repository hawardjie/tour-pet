      ...
      <nav className="bg-white shadow-sm">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16">
            ...
            <Link href="/" className="text-gray-700 hover:text-blue-600">← {t('breeds.backToHome')}</Link>
          </div>
        </div>
      </nav>
      ...
      <div className="bg-white shadow-sm border-b sticky top-0 z-40">
        <div className="max-w-7xl mx-auto px-4 py-4">
          <div className="grid md:grid-cols-3 gap-4">
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">{t('breeds.searchLabel')}</label>
              <input
                type="text"
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">{t('breeds.sizeLabel')}</label>
              <select
                value={sizeFilter}
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">{t('breeds.groupLabel')}</label>
              <select
                value={groupFilter}
      ...
      <div className="max-w-7xl mx-auto px-4 pb-12">
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredBreeds.map(breed => (
            <div key={breed.id} className="bg-white rounded-lg shadow-md hover:shadow-xl transition overflow-hidden">
              ...
              <div className="p-6">
                <div className="flex justify-between items-start mb-3">
                  <h2 className="text-2xl font-bold text-gray-900">{t(`breeds.breed_${breed.id}_name`)}</h2>
                ...
                <p className="text-gray-700 mb-4 line-clamp-3">{t(`breeds.breed_${breed.id}_description`)}</p>

                <div className="mb-4">
                  <h3 className="text-sm font-semibold text-gray-700 mb-2">{t('breeds.temperamentLabel')}</h3>
                ...
                <div className="grid grid-cols-3 gap-2 mb-4 text-center text-xs">
                  <div>
                    <div className="text-gray-500">{t('breeds.exercise')}</div>
                    <div className="font-semibold text-gray-900">{t(`breeds.exercise${getKey(breed.exercise)}`)}</div>
                  </div>
                  <div>
                    <div className="text-gray-500">{t('breeds.grooming')}</div>
                    <div className="font-semibold text-gray-900">{t(`breeds.grooming${getKey(breed.grooming)}`)}</div>
                  </div>
                  <div>
                    <div className="text-gray-500">{t('breeds.training')}</div>
                    <div className="font-semibold text-gray-900">{t(`breeds.training${getKey(breed.training)}`)}</div>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-2 text-xs mb-4">
                  <div className="flex items-center">
                    ...
                    <span className="ml-1 text-gray-600">{t('breeds.goodWithKids')}</span>
                  </div>
                  <div className="flex items-center">
                    <span className={breed.goodWith.dogs ? 'text-green-600' : 'text-red-600'}>
                      {breed.goodWith.dogs ? '✓' : '✗'}
                    </span>
                    <span className="ml-1 text-gray-600">{t('breeds.goodWithDogs')}</span>
                  </div>
                </div>
        ...
        {filteredBreeds.length === 0 && (
          <div className="text-center py-12">
            <p className="text-gray-500 text-lg">{t('breeds.noResults')}</p>
          </div>
        )}
      </div>
    </div>
